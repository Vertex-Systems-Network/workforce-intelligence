...
/** Handles the devices operation for the WorkIntel client. */ export default function Devices() {
    ...
    const deviceColumns = useMemo<DataGridColumn<Device>[]>(() => [
        ...
        { id: 'agent', header: 'Agent', sortValue: row => row.agent_version ?? '', cell: row => <Stack gap={4}><span className="stat-num">v{row.agent_version ?? '—'}</span><Badge tone={row.health === 'healthy' ? 'success' : row.health === 'update_available' ? 'warning' : 'danger'}>{row.health === 'healthy' ? 'Current' : row.health === 'update_available' ? 'Update' : 'Unsupported'}</Badge>{row.health !== 'healthy' && !row.capabilities.includes('self_update') && <span className="ui-card-description">Manual upgrade</span>}</Stack> },
