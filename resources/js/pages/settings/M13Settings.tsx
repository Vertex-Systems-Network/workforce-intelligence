...
import { useConfirmAction, Alert, Badge, Button, Card, CardBody, CardHeader, Field, Input, Modal, Select, Switch, DataGrid, Box, Grid, Inline, Stack, Text, Form, Label, Option, type DataGridColumn } from '../../design-system';
type Preference = {
    id: number;
    category: string;
    in_app: boolean;
...
/** Handles the notification settings m13 operation for the WorkIntel client. */ export function NotificationSettingsM13() {
    ...
    const [prefs, setPrefs] = useState<Preference[]>([]), [loading, setLoading] = useState(true), [saving, setSaving] = useState(false), [message, setMessage] = useState('');
    ...
    return <><Heading title="Notifications" text="Control in-app alerts and optional immediate/digest email delivery per category."/>{message && <Alert tone={message.includes('saved') ? 'success' : 'danger'}>{message}</Alert>}<Card><CardHeader title="Workspace Alerts"/><CardBody>{loading ? <div className="ui-card-description">Loading preferences…</div> : prefs.map((pref, index) => <Box key={pref.category} display="grid" gridColumns="1fr auto auto 150px" align="center" gap={12} p="12px 0" borderBottom="1px solid var(--border-muted)"><div><Box weight={600}>{categories[pref.category] ?? pref.category}</Box><div className="ui-card-description">Operational alerts for {categories[pref.category]?.toLowerCase() ?? pref.category} events</div></div><Label display="flex" align="center" gap={6} size={11}><Switch checked={pref.in_app} onChange={value => setPrefs(rows => rows.map((row, i) => i === index ? { ...row, in_app: value } : row))}/> In app</Label><Label display="flex" align="center" gap={6} size={11}><Switch checked={pref.email} onChange={value => setPrefs(rows => rows.map((row, i) => i === index ? { ...row, email: value } : row))}/> Email</Label><Select value={pref.digest} onChange={e => setPrefs(rows => rows.map((row, i) => i === index ? { ...row, digest: e.target.value as Preference['digest'] } : row))}><Option value="immediate">Immediate</Option><Option value="daily">Daily digest</Option><Option value="weekly">Weekly digest</Option></Select></Box>)}<Inline justify="flex-end" pt={14}><Button variant="primary" loading={saving} onClick={() => void save()}>Save Preferences</Button></Inline></CardBody></Card></>;
}
