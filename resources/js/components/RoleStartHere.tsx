...
/** Render a permission-aware Start Here checklist for the current role or one module subset. */
export default function RoleStartHere({workspace,onNavigate,moduleId,compact=false}:{workspace:AuthWorkspace;onNavigate:(page:Page)=>void;moduleId?:WorkspaceModuleId;compact?:boolean}){
 const experience=useRoleExperience(workspace),guide=experience.guide,{t,text}=useLocalization()
 if(!guide)return null
 const tasks=(moduleId?guide.tasks.filter(task=>workspaceModuleForPage(task.page)?.id===moduleId):guide.tasks).slice(0,compact?4:8)
