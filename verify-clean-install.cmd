...
echo ============================================================
echo WorkIntel - STRICT ZERO INSTALL VERIFICATION
echo ============================================================
echo WARNING: This command DESTROYS the configured database.
echo Use it only on a new/disposable test database.
if /I "%WORKINTEL_RESET_CONFIRM%"=="RESET" goto reset_confirmed
set /p WI_CONFIRM=Type RESET to continue: 
if /I not "%WI_CONFIRM%"=="RESET" (
  echo Cancelled. No database command was run.
  exit /b 1
)
:reset_confirmed
...
call :gate "Disposable database safety guard" "php tools\assert-disposable-database.php" || exit /b 1
for %%D in (storage\framework\views storage\framework\sessions storage\framework\cache\data storage\logs bootstrap\cache) do if not exist "%%D" mkdir "%%D"

findstr /B /C:"DB_CONNECTION=sqlite" .env >nul 2>nul
if not errorlevel 1 if not exist database\database.sqlite type nul > database\database.sqlite

findstr /R /B /C:"APP_KEY=base64:." .env >nul 2>nul
if errorlevel 1 call :gate "Generate application key" "php artisan key:generate --force" || exit /b 1

call :gate "Clear Laravel caches" "php artisan optimize:clear" || exit /b 1
call :gate "Fresh migration and seed" "php artisan migrate:fresh --seed --force" || exit /b 1
call :gate "Second seed pass / idempotency" "php artisan db:seed --force" || exit /b 1
...
call :gate "Seeder integrity" "php tools\audit-seeders.php" || exit /b 1
...
call :gate "Browser full certification" "npm run test:e2e:full" || exit /b 1
...
call :gate "Migration doctor" "php artisan workintel:migration-doctor" || exit /b 1

echo.
echo ============================================================
echo WORKINTEL ZERO INSTALL VERIFICATION PASSED
echo ============================================================
exit /b 0
