...
/** Provides p1 identity seeder behavior within the WorkIntel application. */ class IdentitySeeder extends Seeder
{
    ...
    public function run(): void
    {
        if (app()->environment('production')) {
            throw new \RuntimeException('IdentitySeeder is demo-only and is forbidden in production.');
        }
        $workspace = Workspace::where('slug', 'acme-corp')->first();
        if ($workspace) {
            WorkspaceRegistrationSetting::updateOrCreate(['workspace_id' => $workspace->id], ['mode' => 'invite_only', 'default_role_slug' => 'employee', 'allowed_domains' => ['acme.test'], 'require_email_verification' => true, 'invite_expires_hours' => 168, 'allow_existing_users' => true]);
        }
