...
/** Provides p2 access seeder behavior within the WorkIntel application. */ class AccessControlSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        if (! Schema::hasTable('roles') || ! Schema::hasTable('role_data_scopes')) {
            return;
        }
        ...
        foreach (WorkspaceMember::where('workspace_id', $workspace->id)->with('roles')->get() as $existingMember) {
            if ($existingMember->roles->isEmpty()) {
                continue;
            }
            if ($existingMember->roles->contains(fn ($r) => (bool) ($r->pivot->is_primary ?? false))) {
                continue;
            }
        ...
        if (! $role) {
            $role = app(RoleAccessService::class)->createFromTemplate($workspace, 'project-coordinator', 'Project Coordinator', 'project-coordinator', $workspace->owner_id);
        }
