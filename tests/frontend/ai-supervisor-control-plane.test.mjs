...
test('compact AI supervisor state is machine-readable, bounded and resume-safe', () => {
  ...
  assert.equal(state.timeout_control.fast_batch_default,true)
  assert.equal(state.timeout_control.routine_substep_confirmation_required,false)
...
test('Fast-Batch governance prevents routine conversational fragmentation without weakening gates', () => {
  const agents=read('AGENTS.md')
  const fastBatch=read('docs/ai-state/FAST-BATCH-EXECUTION.md')
  const responseContract=read('docs/ai-state/USER-RESPONSE-CONTRACT.md')
  const options=read('.ai/NEXT-ACTION-OPTIONS.md')
  const claims=parse('docs/ai-state/DETERMINISTIC-CLAIMS.yaml')
  assert.ok(agents.includes('Fast-Batch default'))
  assert.ok(agents.includes('Do not require the user to reply'))
  assert.ok(fastBatch.includes('Never tight-poll'))
  assert.ok(fastBatch.includes('Fast-Batch optimizes interaction count, not safety'))
  assert.ok(responseContract.includes('Fast-Batch response cadence'))
  assert.ok(options.includes('completed, blocked, or waiting milestone handoff'))
  assert.equal(claims.claims.find(x=>x.id==='CLAIM-010')?.value,true)
  assert.equal(claims.claims.find(x=>x.id==='CLAIM-011')?.value,false)
  assert.equal(claims.claims.find(x=>x.id==='CLAIM-012')?.value,false)
})
