...
test('DEV-08 retires the orphan EmployeeProfile demo chain and records the People owner', () => {
  for (const file of [
    'resources/js/pages/EmployeeProfile.tsx',
    'resources/js/data.ts',
    'resources/js/i18n/humanLabels.tsx',
  ]) assert.equal(fs.existsSync(file), false, `${file} must stay retired`)
...
test('DEV-08 dead-source audit is a release gate and keeps intentional standalone assets explicit', () => {
  ...
  for (const marker of ['app.tsx', 'ToolkitPreview.tsx', 'AvatarCropper.tsx', 'console\\.(?:log|debug)', 'setMemberAvatar']) assert.ok(audit.includes(marker), marker)
})
