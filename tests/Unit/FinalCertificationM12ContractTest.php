...
class FinalCertificationM12ContractTest extends TestCase
{
    ...
    public function test_m12_final_certification_contracts_are_present(): void
    {
        ...
        foreach(['self-hosted','Windows','X64','cancel-in-progress: true','ExecutionPolicy Bypass','Bootstrap PowerShell 7 for setup-php','PowerShell/PowerShell/releases/latest','curl.exe','--max-time 300','pdo_sqlite, sqlite3, fileinfo, gd, zip','Ensure Mozilla Firefox system browser','firefox-msi-latest-ssl','INSTALL_MAINTENANCE_SERVICE=false','Parse Laragon release certification script','System.Management.Automation.Language.Parser','Actual Chrome Edge Firefox accessibility certification','test:e2e:cross-browser'] as $marker)$this->assertStringContainsString($marker,$windowsCi);
