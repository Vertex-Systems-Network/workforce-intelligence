...
class FinalCertificationM12ContractTest extends TestCase
{
    ...
    public function test_m12_final_certification_contracts_are_present(): void
    {
        ...
        foreach(['self-hosted','Windows','X64','ExecutionPolicy Bypass','Bootstrap PowerShell 7 for setup-php','PowerShell/PowerShell/releases/latest','pdo_sqlite, sqlite3, fileinfo, gd','Parse Laragon release certification script','System.Management.Automation.Language.Parser','Actual Chrome Edge Firefox accessibility certification','test:e2e:cross-browser'] as $marker)$this->assertStringContainsString($marker,$windowsCi);
