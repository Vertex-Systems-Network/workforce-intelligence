...
/** Provides installation controller behavior within the WorkIntel application. */ class InstallationController extends Controller
{
    ...
    public function index(Request $request, InstallationGuideService $guides): JsonResponse
    {
    ...
    public function show(Request $request, string $guideKey, InstallationGuideService $guides): JsonResponse
    {
        $workspace = $request->attributes->get('workspace');
        $member = $request->attributes->get('workspaceMember');
    ...
    /** Creates create enrollment data for the requested workflow. */
    public function createEnrollment(Request $request, AgentEnrollmentService $enrollments): JsonResponse
    {
        $workspace = $request->attributes->get('workspace');
        $member = $request->attributes->get('workspaceMember');
        $data = $request->validate(['expires_minutes' => ['nullable', 'integer', Rule::in([5, 10, 15, 30, 60])]]);

        return response()->json($enrollments->create($workspace, $member, $request->user(), $data['expires_minutes'] ?? null), 201);
    }

    /** Updates update progress data for the requested resource. */
    public function updateProgress(Request $request, string $guideKey): JsonResponse
    {
        $guide = InstallationGuideCatalog::get($guideKey);
        abort_unless($guide, 404, 'Installation guide not found.');
        $ids = array_column($guide['steps'], 'id');
        ...
        $completed = array_values(array_unique($data['completed_steps']));
        $workspace = $request->attributes->get('workspace');
        $member = $request->attributes->get('workspaceMember');
    ...
    /** Handles the status operation for the current WorkIntel workflow. */
    public function status(Request $request): JsonResponse
    {
        $workspace = $request->attributes->get('workspace');
        $member = $request->attributes->get('workspaceMember');

        return response()->json(['data' => $this->statusPayload($workspace->id, $member->id)]);
    }

    /** Handles the pdf operation for the current WorkIntel workflow. */
    public function pdf(Request $request, string $guideKey, InstallationGuideService $guides, InstallationGuidePdfService $pdf)
    {
        $workspace = $request->attributes->get('workspace');
        $member = $request->attributes->get('workspaceMember');
        $guide = $guides->one($guideKey, $workspace, $member, $request->getSchemeAndHttpHost());
        $bytes = $pdf->render($guide, $request->user()->preferredLocale());
        $filename = 'workintel-'.str_replace('_', '-', strtolower($guideKey)).'-guide.pdf';
    ...
    /** Handles the status payload operation for the current WorkIntel workflow. */
    private function statusPayload(int $workspaceId, int $memberId): array
    {
        $device = Device::where('workspace_id', $workspaceId)->where('member_id', $memberId)->where('status', 'active')->latest('last_seen_at')->first();
        $browser = BrowserConnection::where('workspace_id', $workspaceId)->where('member_id', $memberId)->where('status', 'active')->latest('last_seen_at')->first();
        $shot = Screenshot::where('workspace_id', $workspaceId)->where('member_id', $memberId)->whereNull('deleted_at')->latest('captured_at')->first();
        $activity = ApplicationSession::where('workspace_id', $workspaceId)->where('member_id', $memberId)->latest('ended_at')->first();
