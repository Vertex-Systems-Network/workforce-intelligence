...
class ProductionCertificationDoctor extends Command
{
    ...
    public function handle(): int
    {
        ...
        if ($this->option('require-build') && ! $buildOk) {
            $failed = true;
        }
        ...
        if ($this->option('json')) {
            ...
            foreach ($checks as $name => $check) {
                $this->line(sprintf('[%s] %s — %s', $check['ok'] ? 'PASS' : 'FAIL', $name, $check['detail']));
            }
        }
