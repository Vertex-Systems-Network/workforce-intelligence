...
/** Provides automation engine behavior within the WorkIntel application. */ class AutomationEngine
{
    ...
    /** Handles the installed operation for the current WorkIntel workflow. */
    public function installed(): bool
    {
        return Schema::hasTable('automation_workflows') && Schema::hasTable('automation_runs') && Schema::hasTable('automation_events');
    }

    /** Handles the emit operation for the current WorkIntel workflow. */
    public function emit(Workspace $workspace, string $eventType, array $payload, string $source = 'workspace', ?string $idempotencyKey = null, ?int $onlyWorkflowId = null): ?AutomationEvent
    {
        if (! $this->installed() || ! app(WorkspaceModuleService::class)->shouldProcessBackground($workspace, 'automations') || ! $this->entitlements->allows($workspace, 'feature.automations')) {
            return null;
        }
        if ($idempotencyKey !== null && mb_strlen($idempotencyKey) > 180) {
        ...
        if ($onlyWorkflowId) {
            $query->whereKey($onlyWorkflowId);
        } else {
            $query->whereIn('trigger_type', ['event', 'incoming']);
        }
        foreach ($query->with('actions')->get() as $workflow) {
            if (! $onlyWorkflowId && ! Str::is($workflow->trigger_event ?: '', $eventType)) {
                continue;
            }
            if (! $this->conditions->passes($workflow->conditions ?? [], $workflow->condition_mode ?: 'all', $context)) {
                continue;
            }
    ...
    /** Handles the test operation for the current WorkIntel workflow. */
    public function test(AutomationWorkflow $workflow, array $payload): AutomationRun
    {
        $workflow->loadMissing('actions');
        $workspace = Workspace::findOrFail($workflow->workspace_id);
        $run = $this->createRun($workflow, null, 'automation.test', $payload, ['test' => true]);

        return $this->processRun($run);
    }

    /** Handles the process due operation for the current WorkIntel workflow. */
    public function processDue(int $limit = 100): array
    {
        if (! $this->installed()) {
            return ['scheduled' => 0, 'processed' => 0, 'failed' => 0];
        }
        $scheduled = 0;
        $processed = 0;
        $failed = 0;
        AutomationWorkflow::query()->where('status', 'active')->where('trigger_type', 'schedule')->whereNotNull('next_run_at')->where('next_run_at', '<=', now())->orderBy('next_run_at')->limit(100)->get()->each(function (AutomationWorkflow $workflow) use (&$scheduled) {
            $workspace = Workspace::find($workflow->workspace_id);
            if (! $workspace || ! app(WorkspaceModuleService::class)->shouldProcessBackground($workspace, 'automations') || ! $this->entitlements->allows($workspace, 'feature.automations')) {
                return;
            }
            $due = $workflow->next_run_at;
            $idem = 'schedule:'.$workflow->id.':'.$due?->toIso8601String();
            $event = AutomationEvent::query()->where('workspace_id', $workspace->id)->where('source', 'schedule')->where('idempotency_key', $idem)->first();
            if (! $event) {
                $event = AutomationEvent::create(['uuid' => (string) Str::uuid(), 'workspace_id' => $workspace->id, 'event_type' => 'schedule.'.$workflow->uuid, 'source' => 'schedule', 'idempotency_key' => $idem, 'payload' => ['scheduled_for' => $due?->toIso8601String()], 'occurred_at' => now(), 'processed_at' => now(), 'created_at' => now()]);
                $this->createRun($workflow, $event, 'schedule.'.$workflow->uuid, $event->payload, ['scheduled' => true]);
                $scheduled++;
            }
    ...
    /** Handles the process run operation for the current WorkIntel workflow. */
    public function processRun(AutomationRun $run): AutomationRun
    {
        $run = AutomationRun::query()->with(['workflow.actions.integration', 'workflow'])->findOrFail($run->id);
        $workflow = $run->workflow;
        if (! $workflow) {
            $run->update(['status' => 'failed', 'error' => 'Workflow no longer exists.', 'completed_at' => now()]);

            return $run->fresh();
        }
        $workspace = Workspace::findOrFail($run->workspace_id);
        if (! app(WorkspaceModuleService::class)->shouldProcessBackground($workspace, 'automations')) {
        ...
        foreach ($workflow->actions as $action) {
            if ((microtime(true) - $started) > max(5, (int) $workflow->max_run_seconds)) {
                return $this->failRun($run, 'Workflow exceeded its maximum run time.', $context);
            }
            $rendered = $this->renderer->render($action->config ?? [], $context);
            $step = AutomationRunStep::create(['automation_run_id' => $run->id, 'automation_action_id' => $action->id, 'position' => $action->position, 'name' => $action->name, 'status' => 'running', 'input' => $this->sanitize($rendered), 'attempts' => 0, 'started_at' => now()]);
            $lastError = null;
            $output = null;
            $maxAttempts = max(1, min(6, (int) $action->retry_max + 1));
            for ($attempt = 1; $attempt <= $maxAttempts; $attempt++) {
                $step->attempts = $attempt;
                $step->save();
                try {
                    $output = $this->executeAction($workspace, $action, $rendered, $context);
                    $lastError = null;
                    break;
                } catch (\Throwable $e) {
                    $lastError = $e;
            ...
            if ($lastError) {
                $failedSteps++;
                $step->update(['status' => 'failed', 'completed_at' => now(), 'error' => Str::limit($lastError->getMessage(), 2000, '')]);
                $context['steps'][(string) $action->position] = ['status' => 'failed', 'error' => $lastError->getMessage()];
                if (! $action->continue_on_error && $workflow->failure_policy !== 'continue') {
...
continue;
        ...
        $status = $failedSteps ? 'partial' : 'succeeded';
        $run->update(['status' => $status, 'context' => $this->sanitize($context), 'completed_at' => now(), 'error' => null]);
        $workflow->update(['last_run_at' => now()]);

        return $run->fresh(['steps', 'workflow']);
    }

    /** Handles the retry dead letter operation for the current WorkIntel workflow. */
    public function retryDeadLetter(AutomationDeadLetter $dead, ?int $userId = null): AutomationRun
    {
        $dead->loadMissing('run.workflow');
        abort_unless(! $dead->resolved_at, 422, 'Dead letter is already resolved.');
        $old = $dead->run;
        abort_unless($old && $old->workflow, 422, 'Original workflow is no longer available.');
        ...
        $dead->update(['retry_count' => $dead->retry_count + 1, 'resolved_at' => now(), 'resolved_by' => $userId]);

        return $new;
    }

    /** Creates create run data for the requested workflow. */
    private function createRun(AutomationWorkflow $workflow, ?AutomationEvent $event, string $triggerEvent, array $payload, array $context = []): AutomationRun
    {
    ...
    /** Handles the fail run operation for the current WorkIntel workflow. */
    private function failRun(AutomationRun $run, string $message, array $context): AutomationRun
    {
    ...
    /** Handles the execute action operation for the current WorkIntel workflow. */
    private function executeAction(Workspace $workspace, $action, array $config, array $context): array
    {
    ...
    /** Handles the webhook action operation for the current WorkIntel workflow. */
    private function webhookAction(array $config, int $timeout): array
    {
        $url = (string) ($config['url'] ?? '');
        $httpOptions = $this->urlGuard->httpOptions($url);
        ...
        if (! $response->successful()) {
            throw new \RuntimeException('Webhook action failed with HTTP '.$response->status().': '.Str::limit($response->body(), 700, ''));
        }

        return ['ok' => true, 'status' => $response->status(), 'body' => Str::limit($response->body(), 1200, '')];
    }

    /** Handles the notification action operation for the current WorkIntel workflow. */
    private function notificationAction(Workspace $workspace, array $config): array
    {
        $members = WorkspaceMember::query()->with(['user', 'roles'])->where('workspace_id', $workspace->id)->where('status', 'active');
        $memberIds = array_values(array_filter(array_map('intval', (array) ($config['member_ids'] ?? []))));
        $roles = array_values(array_filter((array) ($config['role_slugs'] ?? [])));
        if ($memberIds) {
            $members->whereIn('id', $memberIds);
        } elseif ($roles) {
            $members->whereHas('roles', fn ($q) => $q->whereIn('slug', $roles));
        } else {
            $members->whereHas('roles', fn ($q) => $q->whereIn('slug', ['owner', 'admin']));
        }
        $count = 0;
    ...
    /** Handles the task action operation for the current WorkIntel workflow. */
    private function taskAction(Workspace $workspace, array $config): array
    {
        $project = Project::query()->where('workspace_id', $workspace->id)->findOrFail((int) ($config['project_id'] ?? 0));
        $creator = (int) $workspace->owner_id;
        if (! $creator) {
            throw new \RuntimeException('Automation task action needs a valid creator.');
        }
        $workflow = app(TaskWorkflowService::class);
        $workflow->ensureDefaults($workspace);
        $status = $workflow->resolveStatus($workspace->id, null, (string) ($config['status'] ?? 'todo'));
        $content = app(TaskContentService::class);
        $description = (string) ($config['description'] ?? '');
        $html = $content->sanitize($config['description_html'] ?? null);
        $plain = $content->plainText($html, $description);
        ...
        $assignees = array_values(array_filter(array_map('intval', (array) ($config['assignee_member_ids'] ?? []))));
        if ($assignees) {
            $valid = WorkspaceMember::query()->where('workspace_id', $workspace->id)->whereIn('id', $assignees)->pluck('id');
            $task->assignees()->sync($valid);
        }
        app(TaskActivityService::class)->log($task, null, 'created', ['source' => 'automation']);

        return ['ok' => true, 'task_id' => $task->id, 'title' => $task->title];
    }

    /** Handles the base context operation for the current WorkIntel workflow. */
    private function baseContext(Workspace $workspace, string $eventType, array $payload, ?AutomationEvent $event): array
    {
