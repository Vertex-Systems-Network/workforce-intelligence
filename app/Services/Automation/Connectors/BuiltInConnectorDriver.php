...
/** Provides built in connector driver behavior within the WorkIntel application. */ class BuiltInConnectorDriver implements ConnectorDriver
{
    ...
    public function __construct(private readonly string $provider, private readonly OutboundUrlGuard $guard) {}

    /** Handles the id operation for the current WorkIntel workflow. */
    public function id(): string
    {
        return $this->provider;
    }

    /** Handles the catalog operation for the current WorkIntel workflow. */
    public function catalog(): array
    {
        return self::definitions()[$this->provider] ?? throw new \InvalidArgumentException('Unknown connector provider.');
    }

    /** Handles the definitions operation for the current WorkIntel workflow. */
    ...
    /** Validates validate config input before it is processed. */
    public function validateConfig(array $config): array
    {
        ...
        foreach ($definition['config_fields'] as $field) {
            ...
            if (($field['required'] ?? false) && ($value === null || trim((string) $value) === '')) {
                throw ValidationException::withMessages(["config.$key" => ["{$field['label']} is required."]]);
            }
            if ($value !== null && $value !== '') {
                $clean[$key] = is_string($value) ? trim($value) : $value;
            }
        }
        if ($this->provider === 'generic_webhook') {
            $this->guard->assertSafe((string) ($clean['url'] ?? ''));
        }
        if (in_array($this->provider, ['slack', 'teams'], true)) {
            $this->guard->assertSafe((string) ($clean['webhook_url'] ?? ''));
        }
        if ($this->provider === 'jira') {
            $this->guard->assertSafe((string) ($clean['base_url'] ?? ''));
        }
        if ($this->provider === 'gitlab' && ! empty($clean['base_url'])) {
            $this->guard->assertSafe((string) $clean['base_url']);
        }
        if ($this->provider === 'github' && ! preg_match('#^[A-Za-z0-9_.-]+/[A-Za-z0-9_.-]+$#', (string) ($clean['repository'] ?? ''))) {
            throw ValidationException::withMessages(['config.repository' => ['Use owner/repository format.']]);
        }
        if ($this->provider === 'quickbooks' && ! in_array($clean['environment'] ?? 'production', ['sandbox', 'production'], true)) {
            throw ValidationException::withMessages(['config.environment' => ['Choose sandbox or production.']]);
        }
    ...
    public function test(array $config, int $timeoutSeconds = 8): array
    {
    ...
    /** Handles the execute operation for the current WorkIntel workflow. */
    public function execute(string $actionKey, array $config, array $input, int $timeoutSeconds = 12): array
    {
        $config = $this->validateConfig($config);
        $allowed = collect($this->catalog()['actions'])->pluck('key')->all();
        if (! in_array($actionKey, $allowed, true)) {
            throw ValidationException::withMessages(['action_key' => ["Action {$actionKey} is not supported by {$this->provider}."]]);
        }
    ...
    /** Handles the execute google operation for the current WorkIntel workflow. */
    private function executeGoogle(string $action, array $config, array $input, int $timeout): array
    {
    ...
    /** Handles the execute microsoft365 operation for the current WorkIntel workflow. */
    private function executeMicrosoft365(string $action, array $config, array $input, int $timeout): array
    {
        $user = rawurlencode($config['user_id'] ?? 'me');
        $base = 'https://graph.microsoft.com/v1.0/'.($user === 'me' ? 'me' : 'users/'.$user);
        $http = Http::timeout($timeout)->withToken($config['access_token'])->acceptJson();
        if ($action === 'mail.send') {
            return $this->response($http->post($base.'/sendMail', ['message' => ['subject' => $input['subject'] ?? 'WorkIntel notification', 'body' => ['contentType' => 'Text', 'content' => $input['body'] ?? ''], 'toRecipients' => [['emailAddress' => ['address' => $input['to'] ?? '']]]]]));
        }
    ...
    /** Handles the execute jira operation for the current WorkIntel workflow. */
    private function executeJira(string $action, array $config, array $input, int $timeout): array
    {
        $base = rtrim($config['base_url'], '/').'/rest/api/3';
        $http = Http::withOptions($this->guard->httpOptions($base))->timeout($timeout)->withBasicAuth($config['email'], $config['api_token'])->acceptJson();
        if ($action === 'comment.create') {
            return $this->response($http->post($base.'/issue/'.rawurlencode($input['issue_key'] ?? '').'/comment', ['body' => $this->jiraDoc((string) ($input['body'] ?? ''))]));
        }
    ...
    /** Handles the execute git hub operation for the current WorkIntel workflow. */
    private function executeGitHub(string $action, array $config, array $input, int $timeout): array
    {
        $base = 'https://api.github.com/repos/'.$config['repository'];
        $http = Http::timeout($timeout)->withToken($config['token'])->withHeaders(['User-Agent' => 'WorkIntel-Automations'])->acceptJson();
        if ($action === 'issue.comment') {
            return $this->response($http->post($base.'/issues/'.(int) ($input['issue_number'] ?? 0).'/comments', ['body' => $input['body'] ?? '']));
        }
    ...
    /** Handles the execute monday operation for the current WorkIntel workflow. */
    private function executeMonday(array $config, array $input, int $timeout): array
    {
        ...
        $columns = $input['column_values'] ?? null;
        if (is_array($columns)) {
            $columns = json_encode($columns, JSON_UNESCAPED_SLASHES);
        }
    ...
    /** Handles the json body operation for the current WorkIntel workflow. */
    private function jsonBody(mixed $value): array
    {
        return is_array($value) ? $value : ['value' => $value];
    }

    /** Handles the jira doc operation for the current WorkIntel workflow. */
    private function jiraDoc(string $text): array
    {
        return ['type' => 'doc', 'version' => 1, 'content' => [['type' => 'paragraph', 'content' => [['type' => 'text', 'text' => $text ?: ' ']]]]];
    }

    /** Handles the quick books base operation for the current WorkIntel workflow. */
    private function quickBooksBase(array $config): string
    {
        return ($config['environment'] ?? 'production') === 'sandbox' ? 'https://sandbox-quickbooks.api.intuit.com' : 'https://quickbooks.api.intuit.com';
    }

    /** Handles the response operation for the current WorkIntel workflow. */
