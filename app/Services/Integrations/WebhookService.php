...
/** Provides webhook service behavior within the WorkIntel application. */ class WebhookService
{
    ...
    /** Handles the queue event operation for the current WorkIntel workflow. */
    public function queueEvent(Workspace $workspace, string $event, array $data): void
    {
        try {
            $this->automations->emit($workspace, $event, $data, 'workspace');
        } catch (\Throwable $e) {
            report($e);
        }
        if (! Schema::hasTable('webhook_endpoints') || ! Schema::hasTable('webhook_deliveries')) {
            return;
        }
        if (! app(EntitlementService::class)->allows($workspace, 'feature.api_access')) {
            return;
        }
        $eventId = (string) Str::uuid();
        foreach (WebhookEndpoint::where('workspace_id', $workspace->id)->where('status', 'active')->get() as $endpoint) {
            if (! $endpoint->accepts($event) && ! $endpoint->accepts('*')) {
                continue;
            }
    ...
    /** Handles the deliver operation for the current WorkIntel workflow. */
    public function deliver(WebhookDelivery $delivery): bool
    {
        ...
        try {
            ...
            $delivery->last_response_excerpt = Str::limit($response->body(), config('workintel_security.webhooks.max_response_excerpt', 900), '');
            ...
            $this->retryOrFail($delivery);
            $endpoint->update(['last_failure_at' => now()]);

            return false;
        } catch (\Throwable $e) {
            $delivery->attempts++;
            $delivery->last_response_excerpt = Str::limit($e->getMessage(), 900, '');
            $this->retryOrFail($delivery);
            $endpoint->update(['last_failure_at' => now()]);
    ...
    /** Handles the retry or fail operation for the current WorkIntel workflow. */
    private function retryOrFail(WebhookDelivery $delivery): void
    {
        ...
        if ($delivery->attempts >= $max) {
            ...
            $delivery->next_attempt_at = now()->addMinutes(min(60, 2 ** $delivery->attempts));
        }
        $delivery->save();
        if ($delivery->status === 'failed') {
            try {
                app(ObservabilityService::class)->record('webhook', 'webhook.delivery_failed', 'Webhook delivery exhausted retries.', 'error', ['delivery_uuid' => $delivery->uuid, 'event_type' => $delivery->event_type, 'endpoint_id' => $delivery->webhook_endpoint_id, 'status_code' => $delivery->last_status_code], $delivery->workspace_id, 'webhook');
            } catch (\Throwable) {
            }
        }
    }
